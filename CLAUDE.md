...
- **Version bumps** are mirrored in two files: `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` (both currently `2.3.0`; note `marketplace.json` carries the version in **two** places — `metadata.version` and `plugins[0].version` — move both). Keep them in sync. (`.codex-plugin/plugin.json` and its `plugins/flow/.codex-plugin/` mirror track their own version independently.)
